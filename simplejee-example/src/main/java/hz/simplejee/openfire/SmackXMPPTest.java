...
public class SmackXMPPTest {
    ...
    public void testUser() {
        try {
            /** 用户登陆，用户名、密码 */
            connection.login("wanghui", "wanghuixx");
        } catch (XMPPException e) {
            e.printStackTrace();
        }
        /** 获取当前登陆用户 */
    ...
    public void testPacket() {
        try {
            connection.login("13671703746", "qazwsx");
        } catch (XMPPException e) {
            e.printStackTrace();
        }
        ...
        Message message = new Message("wangtao@" + server, Type.chat);
    ...
    public void testChatManager() {
        ...
        Chat chat = chatManager.createChat("wangtao@iz23ssjbr39z", new MyMessageListeners());
        try {
            /** 发送消息 */
